...
def retrograde(network, road_type, ego_init, ego_dest, ego_speed, num):
    print("retrograde")
    ...
    while rejection:
        ...
        npc_init_project_length, npc_init_lane = retrograde_init_position(network, npc_dest, npc_dest_project_length, npc_dest_lane)
...
def retrograde_init_position(network, position, npc_dest_project_length, npc_dest_lane):
    npc_init = get_npc_init_point(network, position, relative='front', reverse=None)
    ...
    while (npc_init_project_length - npc_dest_project_length) <= random.randint(5, 50) and npc_init_project_length != npc_dest_lane_length:
        print(npc_init_project_length - npc_dest_project_length)
        print("re")
        npc_init = get_npc_init_point(network, position, relative='front', reverse=None)
        npc_point = Vector(npc_init[0], npc_init[1])
        npc_init_lane = network.laneAt(npc_point)
