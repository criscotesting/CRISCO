...
def cut_out(network, road_type, ego_init, ego_dest, ego_speed, num):
    print("cut out")
    ...
    while rejection:
        ...
        npc_init_project_length, npc_init_lane, center_line, npc_dest_project_length = cut_out_init_position(network, ego_init, npc_dest, npc_dest_lane, ego_init_project_length)
...
def cut_out_init_position(network, position, npc_dest, npc_dest_lane, ego_init_project_length):
    # npc_init = sample_road_points(position)
    # relative = random.choice(['front', 'behind'])
    relative = 'behind'
    npc_init = get_npc_init_point(network, position, same_lane=True, relative=relative)
    ...
    while npc_init_lane == npc_dest_lane or abs(npc_init_project_length-ego_init_project_length) < 5 or abs(npc_init_project_length-ego_init_project_length) > 30 or (npc_dest_project_length - npc_init_project_length) < random.randint(5, 50):
        ...
        print(abs(npc_init_project_length-ego_init_project_length))
...
if __name__ == '__main__':
    network = get_network()
    print(cut_out(network, 'freeway', [-64.1, -118.9], [11.4, -40.6], 10))
